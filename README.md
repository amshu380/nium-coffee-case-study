...
This is my submission for the Nium coffee market entry case. The brief asked which three global markets ACME Baristas should launch in, supported by a data pipeline, SQL transformations, a dashboard, and reflection answers.

The recommendation in one line: **China, Egypt, and Vietnam**, in that order.

## Why these three

China is the obvious size-and-trajectory call. 1.4 billion people, roughly 10.6% five-year CAGR in consumption, urban coffee culture growing fast, and 54% import dependency. The Luckin trajectory says the unit economics work at scale.
...
The dashboard runs locally. Clone the repo, `pip install -r requirements.txt`, then `streamlit run dashboard/app.py`. It will open at localhost:8501. The read-only connection string is shared by email.
...
Two scripts validate the project end to end. `src/audit.py` runs 33 quality checks across data integrity, business logic, statistical sanity, SQL design, dashboard behavior, and submission hygiene. Latest run: 31 pass, 4 warnings, zero failures. `src/verify_dashboard.py` confirms every dashboard filter affects the right tabs. Latest run: 9 of 9 pass.
...
Brazil's number came out to 6.2 kg per person per year, which felt right. Brazilians drink a lot of their own coffee. Good cross-check.

## Known limits to flag

USDA tracks roughly 94 countries, not the whole world.

2025 is a USDA forecast year, not actual. World Bank population stops at 2024. The deep dive headline metrics use the most recent year with population data, while the time series chart extends through 2025.

This is a screening exercise. It produces a defensible short list. It does not replace local consumer research, site visits, or competitor fieldwork.
